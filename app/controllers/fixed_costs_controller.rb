class FixedCostsController < ApplicationController
  ...
  def report
    @fixedCost = FixedCost.all
     # 総固定費
    gon.money_sum = FixedCost.sum(:money)
     # 年毎の総固定費 単位1万円 TO DO DRY原則に反してる感がすごい
     gon.money_year_1 = FixedCost.where(created_at: "2020-01-01".in_time_zone.all_year).sum(:money) / 10000
     gon.money_year_2 = FixedCost.where(created_at: "2019-01-01".in_time_zone.all_year).sum(:money) / 10000
     gon.money_year_3 = FixedCost.where(created_at: "2018-01-01".in_time_zone.all_year).sum(:money) / 10000
    # 月毎の総固定費 単位1万 TO DO DRY原則に反してる感がすごい
    gon.money_month_1 = FixedCost.where(created_at: "2020-01-01".in_time_zone.all_month).sum(:money) / 10000
    gon.money_month_2 = FixedCost.where(created_at: "2020-02-01".in_time_zone.all_month).sum(:money) / 10000
    gon.money_month_3 = FixedCost.where(created_at: "2020-03-01".in_time_zone.all_month).sum(:money) / 10000
    ...
    gon.money_7 = FixedCost.where(created_at: 6.day.ago.all_day).sum(:money) / 10000
    gon.money_6 = FixedCost.where(created_at: 5.day.ago.all_day).sum(:money) / 10000
    gon.money_5 = FixedCost.where(created_at: 4.day.ago.all_day).sum(:money) / 10000
    gon.money_4 = FixedCost.where(created_at: 3.day.ago.all_day).sum(:money) / 10000
    gon.money_3 = FixedCost.where(created_at: 2.day.ago.all_day).sum(:money) / 10000
    gon.money_2 = FixedCost.where(created_at: 1.day.ago.all_day).sum(:money) / 10000
    gon.money_1 = FixedCost.where(created_at: Time.zone.now.all_day).sum(:money) / 10000
    # ラベル
